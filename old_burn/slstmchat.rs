...
fn create_batch<B: AutodiffBackend>(
    ...
    let x = Tensor::<B, 3>::from_inner(
            eye.clone().select(0, indices_inner)
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let eye_input = Tensor::<NdArray, 2>::eye(vocab_size, &device);
    let eye_target = Tensor::<NdArray, 2>::eye(vocab_size, &device);
        ...
        for epoch in 0..num_epochs {
            ...
            for batch_idx in 0..num_batches {
                ...
                let target_one_hot = Tensor::<MyBackend, 2>::from_inner(
                    eye_target.clone().select(0, target_flat.clone().inner())
