...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let hidden_size = 512; // Suficiente para BPE
    let num_layers = 1;//let num_layers = 2;
    let num_blocks = 1; //let num_blocks = 4;
    let output_size = vocab_size; 
    let dropout = 0.1;

    let seq_length = 160; //32 Reducido para evitar explosión de memoria
    let batch_size = 8; // Mucho más seguro para CPU
        ...
        if input.to_lowercase().starts_with("len") {
            ...
            if parts.len() == 1 {
                ...
                match parts[1].parse::<usize>() {
                    ...
                    Ok(_) => {
                        println!("Por favor usa un valor entre 1 y 2000.\n");
                    }
                    Err(_) => {
